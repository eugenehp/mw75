...
async fn main() -> Result<()> {
    ...
    peripheral
        .write(cmd_char, &ENABLE_EEG, WriteType::WithResponse)
        .await?;
    tokio::time::sleep(Duration::from_millis(200)).await;

    info!("Sending ENABLE_RAW…");
    peripheral
        .write(cmd_char, &ENABLE_RAW, WriteType::WithResponse)
        .await?;
    tokio::time::sleep(Duration::from_millis(200)).await;
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let result =
            tokio::task::spawn_blocking(move || try_rfcomm_macos(&name, "A-BLE-connected")).await?;
        match result {
            Ok(channel_info) => {
        ...
        for settle_ms in [1000u64, 2000, 3000, 5000] {
            ...
            let result =
                tokio::task::spawn_blocking(move || try_rfcomm_macos(&name, &label)).await?;
            match result {
                Ok(channel_info) => {
        ...
        if let Some(cmd) = chars2.iter().find(|c| c.uuid == CMD_1101) {
            peripheral2
                .write(cmd, &ENABLE_EEG, WriteType::WithResponse)
                .await?;
            tokio::time::sleep(Duration::from_millis(200)).await;
            peripheral2
                .write(cmd, &ENABLE_RAW, WriteType::WithResponse)
                .await?;
            tokio::time::sleep(Duration::from_millis(500)).await;
        ...
        let name = device_name.clone();
        let result =
            tokio::task::spawn_blocking(move || try_rfcomm_macos(&name, "C-BLE-reactivated"))
                .await?;
        match result {
            Ok(channel_info) => {
...
fn try_rfcomm_macos(device_name: &str, label: &str) -> Result<String> {
    ...
    if !is_connected {
        ...
        for i in 0..20 {
            ...
            if c {
                info!(
                    "[{label}] Baseband connected after {:.1}s",
                    (i + 1) as f64 * 0.5
                );
                break;
            }
    ...
    if !try_order.contains(&25) {
        try_order.insert(0, 25);
    }
    ...
    Err(anyhow!(
        "All RFCOMM attempts failed (channels tried: {try_order:?})"
    ))
}
    ...
    loop {
        ...
        match data_rx.try_recv() {
            Ok(data) => {
                ...
                if packet_count < 10 {
                    let h = if data.len() <= 64 {
                        hex(&data)
                    } else {
                        format!("{} … ({} B)", hex(&data[..32]), data.len())
                    };
                ...
                for event in &events {
                    if let mw75::types::Mw75Event::Eeg(pkt) = event {
                        if packet_count < 20 {
                            info!(
                                "  ✅ EEG: counter={} ref={:.2} drl={:.2} ch1={:.2}µV",
                                pkt.counter,
                                pkt.ref_value,
                                pkt.drl,
                ...
                if packet_count > 0 && packet_count.is_multiple_of(500) {
                    ...
                    let elapsed = 30.0
                        - deadline
                            ...
                            .as_secs_f64();
                    info!(
                        "  📊 {packet_count} pkts, {total_bytes} B, {:.1} Hz, err={:.1}%",
...
fn macos_rfcomm_with_delegate(
    ...
    // Delegate setup
    thread_local! {
        static DATA_SENDER: RefCell<Option<Sender<Vec<u8>>>> = const { RefCell::new(None) };
    }
    DATA_SENDER.with(|cell| {
        *cell.borrow_mut() = Some(data_tx.clone());
    });
    ...
    let delegate_class = unsafe {
        ...
        if let Some(cls) = existing {
            ...
            builder.add_method(
                ...
                rfcomm_data_cb
                    as extern "C" fn(*mut AnyObject, Sel, *const AnyObject, *const c_void, usize),
            );
            builder.add_method(
...
async fn rfcomm_data_probe_linux(_device_name: &str) -> Result<()> {
    ...
    loop {
        ...
        match tokio::time::timeout(Duration::from_secs(5), stream.read(&mut buf)).await {
            ...
            Ok(Ok(n)) => {
                ...
                if packet_count < 10 {
                    info!("  chunk ({n} B): {}", hex(&buf[..n.min(64)]));
                }
                for event in processor.process_data(&buf[..n]) {
                    if let mw75::types::Mw75Event::Eeg(pkt) = &event {
                        if packet_count < 20 {
                            info!(
                                "  ✅ EEG: counter={} ch1={:.2}µV",
                                pkt.counter,
                                pkt.channels.first().unwrap_or(&0.0)
                            );
                        }
    ...
    info!("\n═══ Summary ═══");
    info!(
        "  Bytes: {total_bytes}  Packets: {packet_count}  Error: {:.1}%",
        stats.error_rate()
    );
    Ok(())
}
